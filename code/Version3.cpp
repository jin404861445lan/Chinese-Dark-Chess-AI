...
int kill(int id, int depth, int alpha, int beta, int score) {
    ...
    for (int i = 1; i <= current_num[id]; ++i) {
        ...
        if (!ok_to_kill(id ^ 1, chess) || (OrderTab[chess][28] < 0) || (OrderTab[chess][12] < 0)) continue;
...
inline void my_treat() {
    ...
    if (noteaten[id ^ 1] <= 0) {
        ...
        if (noteaten[id ^ 1] == 2) MAX_DEPTH = 6;
        if (noteaten[id ^ 1] == 1) MAX_DEPTH = 6;
    	...
    	if ((walk_val < unwalk_val && can_flip()) || dec_pos == -1 || (walk_val <= unwalk_val + 200 && can_flip() && rand() % 100 <= 1))
        	do_flip();
