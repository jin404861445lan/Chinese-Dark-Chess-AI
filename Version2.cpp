...
int AlphaBeta(int id, int depth, int alpha, int beta, int score) {
    ...
    for (int i = 1; i <= current_num[id]; ++i) {
        ...
        if (!ok_to_kill(id ^ 1, chess) && noteaten[id ^ 1] <= 2) continue;
...
inline void my_treat() {
    ...
    MAX_DEPTH = noteaten[id ^ 1] <= 2 ? 10 : 5;
    if (noteaten[id ^ 1] == 3) MAX_DEPTH = 8;
    ...
    if ((walk_val < unwalk_val && can_flip()) || dec_pos == -1 || (walk_val <= unwalk_val + 200 && can_flip() && rand() % 100 <= 2))
        do_flip();
